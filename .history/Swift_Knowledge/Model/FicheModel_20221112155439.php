...
class FicheModel
{
    ...
    {
        $requete = ConnexionBdd::getInstance()->prepare("SELECT T1.TITRE as ftitle ,T1.IDFICHE ,T1.TEXTEFICHE, T1.DATE ,T1.IDFICHE ,T2.NOM , T2.PRENOM, T3.TITRE FROM ficheconnaissance T1 INNER JOIN user T2 ON T1.IDUSER = T2.IDUSER INNER JOIN categorie T3 ON T1.IDCAT = T3.IDCAT WHERE T1.IDFICHE = 1");
        $requete->execute(array($id));
    ...
    {
        $requete = ConnexionBdd::getInstance()->prepare("SELECT user.NOM, user.PRENOM , commentaire.IDCOMMENTAIRE ,commentaire.CONTENU ,commentaire.IDFICHE FROM commentaire INNER JOIN ficheconnaissance ON ? = ficheconnaissance.IDFICHE INNER JOIN user ON user.IDUSER = commentaire.IDUSER;");
